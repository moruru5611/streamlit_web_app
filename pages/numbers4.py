...
def numbers4_simulator_customize(num1,num2,num3,num4):
    numbers4=random.sample(range(0,10),4)
    your_numbers=[num1,num2,num3,num4]
    straight=[]
    mini=[]
    #ストレート判定
    for n,y in zip(numbers4,your_numbers):
        if n==y:
            straight.append(n)
        if len(straight)>=0:
            mini.append(n)
    #ボックス判定
    box=set(numbers4) & set(your_numbers)
    #ミニ判定
    mini_result="ミニ当選" if len(mini)==2 else "該当なし"
    print("NUMBERS4抽選結果（改良版）")
    print("当たり番号：",numbers4)
    print("あなたの番号：",your_numbers)
    print("ストレート",straight)
    print("ボックス",list(box))
    print("ミニ：",mini_result)
    print("----------------------------")
    return numbers4,your_numbers,straight,box,mini_result

st.title("NUMBERS4シミュレーター")
...
if submit_btn:
  numbers4,your_numbers,straight,box,mini_result=numbers4_simulator_customize(num1,num2,num3,num4)
  st.write(f"当たり番号は{numbers4}です")
